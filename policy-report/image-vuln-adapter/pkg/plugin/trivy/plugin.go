...
func (p *plugin) ParseVulnerabilityReportData(ctx imgvuln.PluginContext, imageRef string, logsReader io.ReadCloser) (v1alpha1.VulnerabilityReportData, error) {
	config, err := p.newConfigFrom(ctx)
	if err != nil {
	...
	version, err := imgvuln.GetVersionFromImageRef(trivyImageRef)
	if err != nil {
		return v1alpha1.VulnerabilityReportData{}, err
	}
