...
func GetContainerImagesFromJob(job *batchv1.Job) (ContainerImages, error) {
	...
	if containerImagesAsJSON, ok = job.Annotations[imgvuln.AnnotationContainerImages]; !ok {
		return nil, fmt.Errorf("required annotation not set: %s", imgvuln.AnnotationContainerImages)
	}
	containerImages := ContainerImages{}
	err := containerImages.FromJSON(containerImagesAsJSON)
	if err != nil {
		return nil, fmt.Errorf("parsing annotation: %s: %w", imgvuln.AnnotationContainerImages, err)
	}
