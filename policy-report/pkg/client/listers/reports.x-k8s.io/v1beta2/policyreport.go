...
type PolicyReportLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.PolicyReport, err error)
...
// List lists all PolicyReports in the indexer.
func (s *policyReportLister) List(selector labels.Selector) (ret []*v1beta2.PolicyReport, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.PolicyReport))
	})
	return ret, err
}
...
type PolicyReportNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.PolicyReport, error)
...
// List lists all PolicyReports in the indexer for a given namespace.
func (s policyReportNamespaceLister) List(selector labels.Selector) (ret []*v1beta2.PolicyReport, err error) {
...
// Get retrieves the PolicyReport from the indexer for a given namespace and name.
func (s policyReportNamespaceLister) Get(name string) (*v1beta2.PolicyReport, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("policyreport"), name)
	}
	return obj.(*v1beta2.PolicyReport), nil
}
