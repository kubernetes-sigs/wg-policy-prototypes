...
type ClusterPolicyReportLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta2.ClusterPolicyReport, err error)
	// Get retrieves the ClusterPolicyReport from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta2.ClusterPolicyReport, error)
...
// List lists all ClusterPolicyReports in the indexer.
func (s *clusterPolicyReportLister) List(selector labels.Selector) (ret []*v1beta2.ClusterPolicyReport, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta2.ClusterPolicyReport))
	})
	return ret, err
}

// Get retrieves the ClusterPolicyReport from the index for a given name.
func (s *clusterPolicyReportLister) Get(name string) (*v1beta2.ClusterPolicyReport, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta2.Resource("clusterpolicyreport"), name)
	}
	return obj.(*v1beta2.ClusterPolicyReport), nil
}
