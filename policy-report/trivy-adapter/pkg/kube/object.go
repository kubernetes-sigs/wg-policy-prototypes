...
func ObjectFromLabelsSet(set labels.Set) (Object, error) {
	if !set.Has(imgvuln.LabelResourceKind) {
		return Object{}, fmt.Errorf("required label does not exist: %s", imgvuln.LabelResourceKind)
	}
	if !set.Has(imgvuln.LabelResourceName) {
		return Object{}, fmt.Errorf("required label does not exist: %s", imgvuln.LabelResourceName)
	}
	return Object{
		Kind:      Kind(set.Get(imgvuln.LabelResourceKind)),
		Name:      set.Get(imgvuln.LabelResourceName),
		Namespace: set.Get(imgvuln.LabelResourceNamespace),
	}, nil
}
